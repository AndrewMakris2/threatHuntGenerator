...
export default function HuntGenerator() {
  const { state, dispatch, syncDispatch, addToast } = useApp();
  ...
  async function handleGenerate() {
    ...
    try {
      const aiSettings = (state.aiSettings?.apiKey || state.aiSettings?.provider === 'local')
        ? state.aiSettings
        : null;

      const result = await generateHunts(
        useProfile,
        { maxHunts, categories: selectedCategories },
        aiSettings,
      );

      const hunts = result.hunts ?? result; // handle legacy plain array just in case
      ...
      const session = {
        id: `session-${Date.now()}`,
        companyId: state.activeCompanyId || null,
        companyName: useProfile.companyName || 'Unknown Company',
      ...
      syncDispatch({ type: ACTIONS.ADD_HUNT_SESSION, session });

      if (result.method === 'ai') {
        addToast(`✦ ${hunts.length} hunts generated by ${result.provider} AI`, 'success');
      } else if (result.method === 'fallback') {
        addToast(`AI failed — ${result.error || 'unknown error'}. Showing rules-engine hunts instead.`, 'warning');
      } else {
      ...
      <div className="hunt-generator-layout">
        ...
        <div className="hunt-generator-config">
          ...
          <div className={`card ${profileComplete ? 'card-accent' : ''} hunt-generator-profile-card`}>
            ...
            {profileComplete && (
              <button className="btn btn-ghost btn-sm" onClick={() => navigate('/companies')} style={{ fontSize: '0.75rem' }}>
                Switch Company
              </button>
            )}
          </div>
          ...
          <div className="card">
            ...
            <p className="form-hint">
              {state.aiSettings?.apiKey
                ? `AI is active — hunts will be generated using ${state.aiSettings.provider} (${state.aiSettings.model || 'default model'}).`
                : 'Connect an AI provider in Settings to enable live AI-generated hunts.'}
            </p>
            {showPrompt && (
            ...
            {isGenerating && (
              <div className="hunt-generator-progress">
                <div className="hunt-generator-progress-steps">
                  {(state.aiSettings?.apiKey
                    ? ['Building environment prompt...', 'Calling AI provider...', 'Parsing AI response...', 'Enriching with threat context...']
                    : ['Analyzing environment...', 'Selecting templates...', 'Personalizing hunts...', 'Scoring relevance...']
                  ).map((step, i) => (
          ...
          <div className="card hunt-generator-how">
            ...
            <div className="hunt-generator-how-steps">
              {[
                ...
                { n: '2', t: state.aiSettings?.apiKey ? 'AI Generation' : 'Template Selection', d: state.aiSettings?.apiKey ? 'Sends your full environment profile to the configured AI provider to write custom hunts' : 'Scores 12+ hunt templates by relevance to your profile' },
                { n: '3', t: 'Personalization', d: state.aiSettings?.apiKey ? 'Queries written specifically for your SIEM syntax, EDR platform, and data types' : 'Injects your actual tool names, data types, and threat context' },
